---
name: Rent Cycle Frontend Documentation
overview: Create a comprehensive markdown documentation file for the frontend team covering the new Rent Cycle (Invoice) architecture, including API endpoints, UI/UX recommendations, status display locations, action button placements, and workflow diagrams.
todos:
  - id: create-doc-file
    content: Create RENT_CYCLE_FRONTEND_GUIDE.md file with comprehensive documentation
    status: completed
  - id: add-architecture-diagrams
    content: Add mermaid diagrams for status flow and data relationships
    status: completed
    dependencies:
      - create-doc-file
  - id: add-ui-examples
    content: Add detailed UI component examples and code snippets
    status: completed
    dependencies:
      - create-doc-file
  - id: add-api-examples
    content: Add complete API integration examples with TypeScript/React code
    status: completed
    dependencies:
      - create-doc-file
---
...
All endpoints require authentication via cookie-based session (`access_token` cookie).**Role Requirements:**
...
- Users can only access rent cycles for companies they belong to
- Tenants can only see their own invoices
- Admins/Managers can see all invoices for their companies
...
**Required Role**: `COMPANY_ADMIN` or `MANAGER`**Use Case**: Manually create an invoice (rare, usually automatic).**Request Body:**
...
**Required Role**: `COMPANY_ADMIN` or `MANAGER`**Use Case**: Manually apply late fee to an overdue invoice.**Note**: Late fees are typically applied automatically by the system when an invoice becomes overdue (past grace period). This endpoint allows manual application if needed.**Response:**
...
**New Field**: `rentCycleId` (optional, UUID) - Link payment to specific invoice**Payment Auto-Linking Logic:**If `rentCycleId` is not provided but `period` and `leaseId` are provided, the system will automatically:
...
**New Query Parameter**: `rentCycleId` (optional, UUID) - Filter payments by invoice**Use Case**: Show all payments for a specific invoice on the invoice detail page.**Example:**
...
**New Section: "Invoices" Tab**Display all Rent Cycles for this lease:
...
**Recommended Location**: Main navigation or under "Payments" section**Views:**
...
**Current**: Shows payment history**Enhancement**: Add "Invoices" view or tab**New Features:**
...
**Scenario**: Tenant sees invoice with status "DUE"**Steps:**
...
**Scenario**: User wants to see all invoices for a lease**Steps:**
...
**Scenario**: Tenant pays part of invoice amount**Steps:**
...
**Scenario**: Tenant wants to see what's due soon**Steps:**
...
**Scenario**: Admin needs to see all overdue invoices**Steps:**
